...
- **侧栏收纳**：点栏目**右缘的那个小圆钮**（张开时是 ‹，缩进时是 ›）或按 **Ctrl+B**
  把左侧栏整个收起来，给正文让出空间；缩进后圆钮跟着停在窗口左缘，点一下就能拉回来。
  折叠状态会被记住，下次打开还是原样
- **正文宽度可调**：鼠标移到正文栏**左缘或右缘附近**，栏缘会浮出一个小圆条（平时看不见），
  拖它就能把内容拉宽 / 收窄（一屏看更多），**双击**复位为默认宽度；宽度会被记住——窗口变小时
  正文栏自动跟着收窄，窗口再变宽就回到你调过的宽度。手机 / 平板等没有鼠标的设备用不上拖拽，
  正文栏直接铺满整屏
...
- **快捷浏览**：`←` / `→` 在文件间切换；`Ctrl+B` 显示 / 隐藏侧栏；工具栏提供上一个 / 下一个、源文本视图、复制原文；正文宽度手柄聚焦时，`←` / `→` 微调宽度（Shift 更细，Home / End 到最窄 / 最宽）
